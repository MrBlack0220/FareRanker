...
from transformers import RobertaTokenizer,GPT2Tokenizer, T5ForConditionalGeneration,GPTNeoForCausalLM
...
def main(args):
    ...
    for index, problem in tqdm(enumerate(problems), ncols=0, total=len(problems)):
        ...
        input_id = tokenizer.encode(input_text, verbose=False)
        if len(input_id) > 600:
            input_id = input_id[:595] + tokenizer.encode("\nANSWER:\n",verbose=False)
            
        with torch.no_grad():
            input_ids = torch.LongTensor(input_id).unsqueeze(0).cuda()
            ...
            print("done")
            fina_output_programs = []
            for output_program in output_programs:
                 fina_output_programs.append(output_program.split("ANSWER:\n")[1].replace("<|endoftext|>", ""))
            saved_codes = {}
            saved_codes[problem_id] = {'code': fina_output_programs, 'prompt': input_text}
