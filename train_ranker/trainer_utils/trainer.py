...
class Trainer:
    ...
    def train(
        ...
        **kwargs,
        ...
        train_dataloader_1 = self.get_train_dataloader()
        train_dataloader_2 = self.get_train_dataloader()
        # Setting up training control variables:
        # number of training epochs: num_train_epochs
        # number of training steps per epoch: num_update_steps_per_epoch
        # total number of training steps to execute: max_steps
        total_train_batch_size = args.train_batch_size * args.gradient_accumulation_steps * args.world_size  # 12=2*3*2
        if train_dataset_is_sized:
            num_update_steps_per_epoch = len(train_dataloader_1) // args.gradient_accumulation_steps #len(train_dataloader)共包括多少批次的数据,所以这里求的是真正的梯度更新的次数
            ...
            if args.max_steps > 0:
                ...
                max_steps = math.ceil(args.num_train_epochs * num_update_steps_per_epoch)   # 所有epoch的一共更新步骤
                num_train_epochs = math.ceil(args.num_train_epochs)
                num_train_samples = len(self.train_dataset) * args.num_train_epochs  # 一共训练的样本个数
        else:
            # see __init__. max_steps is set when the dataset has no __len__
            max_steps = args.max_steps  # args.max_steps对应一个epoch更新的步骤
        ...
        elif not delay_optimizer_creation:  # 走此
        ...
        # Train!
        num_examples = (
            self.num_examples(train_dataloader_1) if train_dataset_is_sized else total_train_batch_size * args.max_steps
        )
        ...
        self.callback_handler.train_dataloader = train_dataloader_1
        ...
        # TODO:
        tr_loss_1 = torch.tensor(0.0).to(args.device)
        tr_loss_2 = torch.tensor(0.0).to(args.device)
        # _total_loss_scalar is updated everytime .item() has to be called on tr_loss and stores the sum of all losses
        # TODO:
        self._total_loss_scalar_1 = 0.0
        self._total_loss_scalar_2 = 0.0
        ...
        if not args.ignore_data_skip:
            for epoch in range(epochs_trained):
                ...
                for _ in train_dataloader_1:
                    break
        last_memory = 0            
        for epoch in range(epochs_trained, num_train_epochs):
            # TODO:reset_epoch
            if epoch != 0:
                train_dataloader_1.dataset.read_data(epoch+1)  
                train_dataloader_2.dataset.read_data(epoch+1)
            if isinstance(train_dataloader_1, DataLoader) and isinstance(train_dataloader_1.sampler, DistributedSampler):
                train_dataloader_1.sampler.set_epoch(epoch)  
                train_dataloader_2.sampler.set_epoch(epoch)
            elif isinstance(train_dataloader_1.dataset, IterableDatasetShard):
                train_dataloader_1.dataset.set_epoch(epoch)

            if is_torch_tpu_available():
                parallel_loader = pl.ParallelLoader(train_dataloader, [args.device]).per_device_loader(args.device)
                epoch_iterator = parallel_loader
            else:
                epoch_iterator = train_dataloader_1
            ...
            step2_step = -1
            for step, inputs in enumerate(train_dataloader_1): 
                ...
                if (
                    ...
                    with model.no_sync():
                        tr_loss_1 += self.training_step(model, inputs[0])
                else:
                    tr_loss_1 += self.training_step(model, inputs[0])
                self.current_flos += float(self.floating_point_ops(inputs[0]))    
                if self.deepspeed:
                    self.deepspeed.step()
                if (step + 1) % args.gradient_accumulation_steps == 0 or (
                    ...
                    if self.deepspeed:
                        ...
                        self.optimizer.step()
                    model.zero_grad()
                    for inputs in train_dataloader_2:
                        step2_step += 1                                          
                        if (
                            ((step2_step + 1) % args.gradient_accumulation_steps != 0)
                            and args.local_rank != -1
                            and args._no_sync_in_gradient_accumulation
                        ):
                            with model.no_sync():
                                tr_loss_2 += self.training_step(model, inputs[1])
                        else:
                            tr_loss_2 += self.training_step(model, inputs[1])
                        self.current_flos += float(self.floating_point_ops(inputs[1]))    
                        if self.deepspeed:
                            self.deepspeed.step()
                        if (step2_step + 1) % args.gradient_accumulation_steps == 0 or (
                            # last step in epoch but step is always smaller than gradient_accumulation_steps
                            steps_in_epoch <= args.gradient_accumulation_steps  
                            and (step2_step + 1) == steps_in_epoch
                        ):
                            if args.max_grad_norm is not None and args.max_grad_norm > 0 and not self.deepspeed:
                                # deepspeed does its own clipping
                                if self.use_amp:
                            ...
                            self._maybe_log_save_evaluate(tr_loss_1,tr_loss_2, model, trial, epoch)
                            break
        ...
        # TODO
        self._total_loss_scalar_1 += tr_loss_1.item()
        self._total_loss_scalar_2 += tr_loss_2.item()
        train_loss_1 = self._total_loss_scalar_1 / self.state.global_step
        train_loss_2 = self._total_loss_scalar_2 / self.state.global_step

        metrics = speed_metrics("train", start_time, num_samples=num_train_samples, num_steps=self.state.max_steps)
        self.store_flos()
        metrics["total_flos"] = self.state.total_flos
        metrics["train_loss_step1"] = train_loss_1
        metrics["train_loss_step2"] = train_loss_2
        self.is_in_train = False
        ...
        return TrainOutput(self.state.global_step, train_loss_1,train_loss_2, metrics)
    ...
    def _maybe_log_save_evaluate(self, tr_loss_1,tr_loss_2, model, trial, epoch):
        if self.control.should_log:
            logs: Dict[str, float] = {}
            tr_loss_scalar_1 = tr_loss_1.item()
            tr_loss_scalar_2 = tr_loss_2.item()
            # reset tr_loss to zero
            tr_loss_1 -= tr_loss_1
            tr_loss_2 -= tr_loss_2

            logs["step1_loss"] = round(tr_loss_scalar_1 / (self.state.global_step - self._globalstep_last_logged), 4)
            logs["step2_loss"] = round(tr_loss_scalar_2 / (self.state.global_step - self._globalstep_last_logged), 4)
            logs["learning_rate"] = self._get_learning_rate()

            self._total_loss_scalar_1 += tr_loss_scalar_1
            self._total_loss_scalar_2 += tr_loss_scalar_2
            self._globalstep_last_logged = self.state.global_step
            self.store_flos()
    ...
    def evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):
            ...
            # Prediction step
            ids,logits,results= self.prediction_step(model, inputs, prediction_loss_only, ignore_keys=ignore_keys)
            # loss: loss, preds: list of selected sequences, targets: the ground truth sequences[1.2332, 1.1498, 1.1330, 1.1738]

            # Update containers on host
            if ids is not None:
                ids = self._nested_gather_object(ids) # Gather value of python objects
                ids_host = ids if ids_host is None else ids_host + ids  #存储所有的ids
            # if bianhao is not None:
            #     bianhao = self._nested_gather_object(bianhao)
            #     bianhao_host = bianhao if bianhao_host is None else bianhao_host + bianhao  #存储所有的bianhao
            if logits is not None:
                # logits = self._pad_across_processes(logits)
                logits = self._nested_gather(logits)
                preds_host = logits if preds_host is None else torch.cat((preds_host,logits),dim=0)  #二维列表 存储排序后的正误值
            if results is not None:
                # labels = self._pad_across_processes(labels)
                results = self._nested_gather_object(results)
                results_host = results if results_host is None else results_host + results
            ...
            if self.args.eval_accumulation_steps is not None and (step + 1) % self.args.eval_accumulation_steps == 0:
                if ids_host is not None:
                    ...
                    # all_ids = ids if all_ids is None else np.concatenate((all_ids, ids), axis=0)
                    all_ids = ids_host if all_ids is None else all_ids + ids_host
                if preds_host is not None:
                    preds_host = nested_numpify(preds_host)
                    all_preds = preds_host if all_preds is None else np.concatenate((all_preds,preds_host),axis=0)
                if results_host is not None:
                    all_results = results_host if all_results is None else all_results + results_host

                # Set back to None to begin a new accumulation
                ids_host, preds_host, results_host = None, None, None
##########################################################################################################################
        ...
        if preds_host is not None:
            ...
            all_preds = preds if all_preds is None else np.concatenate((all_preds,preds_host),axis=0)
        if results_host is not None:
            all_results = results_host if all_results is None else all_results + results_host
        # if bianhao_host is not None:
        #     all_bianhao = bianhao_host if all_bianhao is None else all_bianhao + bianhao_host
        ...
        if self.compute_metrics is not None and all_preds is not None:
            ...
            metrics = self.compute_metrics(all_ids,all_preds,all_results)
        else:
            metrics = {}
    ...
    def prediction_step(
        ...
        inputs = self._prepare_inputs(inputs)  #将是tensor的数据类型放到对应的GPU上
        ...
        # else:
        #     labels = None
        input_results = inputs.pop('results')  # list len(input_results) == B
        input_ids = inputs.pop('ids')  # list len(input_ids) == B
        # inputs = 
        ...
        with torch.no_grad():
            if is_sagemaker_mp_enabled():
                ...
                logits = model.evaluate(**inputs)  # (B,)
                loss = None
        ...
        logits = nested_detach(logits) # logits detach计算图(B, )
        # if len(logits) == 1:
        # logits = logits.cpu().numpy() # (B, )
        
        # ordered_list = np.argsort(logits, axis=1)

        # fina_result = []
        # for i in range(ordered_list.shape[0]): # 遍历每个样本
        #     tmp = []
        #     for j in range(ordered_list.shape[1]): # 遍历每个候选
        #         tmp.append(sample_data[i][ordered_list[i][j]])
        #     fina_result.append(tmp)    

        return (input_ids,logits,input_results)
